...
public class Reducer13 extends Reducer<Text,Text,Text,Text>{
	...
	IOException, InterruptedException {
		...
		for(Text val: values){
			...
			if(author_TFIDF.length == 2){
				context.getCounter(PROJECT_COUNTER.MAP_SIZE).setValue(tempVals.size());
		...
		output.write("job13output", new Text(completeVector), new Text(""));
	}
}